...
function AdminGrievances() {
  ...
  const load = async () => {
    const { data } = await supabase
      .from("grievances")
      .select("*, profile:profiles!grievances_farmer_id_fkey(full_name, phone, village)")
  ...
  const update = async (id: string, status: string) => {
    const { error } = await supabase
      .from("grievances")
      .update({ status: status as never, admin_response: responses[id] ?? null })
      .eq("id", id);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Auto-categorised and ranked by urgency. High-priority cases appear at the top of the inbox.
        </p>
      </div>
              ...
              {open === g.id && (
                <div className="space-y-4 border-t border-border bg-background/50 px-5 py-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Detail label="Phone" value={g.profile?.phone ?? "—"} />
                    <Detail label="Filed" value={new Date(g.created_at).toLocaleString()} />
                  </div>
                  <div>
                    <div className="text-xs font-medium text-muted-foreground">Response to farmer</div>
                    <Textarea
                      rows={3}
                      defaultValue={g.admin_response ?? ""}
                      placeholder="Write a response or note for record…"
                      onChange={(e) => setResponses((r) => ({ ...r, [g.id]: e.target.value }))}
                      className="mt-1"
                    />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button size="sm" variant="outline" onClick={() => update(g.id, "in_progress")}>
                      Mark in progress
                    </Button>
                    <Button size="sm" onClick={() => update(g.id, "resolved")} className="gap-1.5">
                      <CheckCircle2 className="h-4 w-4" /> Resolve
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => update(g.id, "closed")}>
                      Close
                    </Button>
                  </div>
                </div>
